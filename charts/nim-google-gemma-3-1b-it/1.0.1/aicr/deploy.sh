...
BUNDLE_NAMESPACES=$(echo "nim-google-gemma-3-1b-it " | tr ' ' '\n' | sort -u | tr '\n' ' ')
...
echo "Installing gemma-3-1b-it (nim-google-gemma-3-1b-it)..."
COMPONENT_WAIT_ARGS="${WAIT_ARGS}"
if echo "${ASYNC_COMPONENTS}" | grep -qw "gemma-3-1b-it"; then
  COMPONENT_WAIT_ARGS=""
...
helm upgrade --install gemma-3-1b-it gemma-3-1b-it \
  ...
  -f "${SCRIPT_DIR}/gemma-3-1b-it/values.yaml" \
  ${COMPONENT_WAIT_ARGS} \
  || helm_failed "gemma-3-1b-it"
