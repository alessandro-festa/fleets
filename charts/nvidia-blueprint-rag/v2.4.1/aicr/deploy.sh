...
BUNDLE_NAMESPACES=$(echo "nvidia-blueprint-rag nvidia-blueprint-rag nvidia-blueprint-rag nvidia-blueprint-rag nvidia-blueprint-rag nvidia-blueprint-rag " | tr ' ' '\n' | sort -u | tr '\n' ' ')
...
echo "Installing main (nvidia-blueprint-rag)..."
COMPONENT_WAIT_ARGS="${WAIT_ARGS}"
if echo "${ASYNC_COMPONENTS}" | grep -qw "main"; then
  COMPONENT_WAIT_ARGS=""
...
helm upgrade --install main main \
  ...
  -f "${SCRIPT_DIR}/main/values.yaml" \
  ${COMPONENT_WAIT_ARGS} \
  || helm_failed "main"
echo "Installing nv-ingest (nvidia-blueprint-rag)..."
COMPONENT_WAIT_ARGS="${WAIT_ARGS}"
if echo "${ASYNC_COMPONENTS}" | grep -qw "nv-ingest"; then
  COMPONENT_WAIT_ARGS=""
  echo "  (async component â skipping --wait)"
fi
helm upgrade --install nv-ingest oci://dp.apps.rancher.io/charts/nv-ingest/nv-ingest \
  --version * \
  -n nvidia-blueprint-rag --create-namespace \
  -f "${SCRIPT_DIR}/nv-ingest/values.yaml" \
  ${COMPONENT_WAIT_ARGS} \
  || helm_failed "nv-ingest"
echo "Installing eck-elasticsearch (nvidia-blueprint-rag)..."
COMPONENT_WAIT_ARGS="${WAIT_ARGS}"
if echo "${ASYNC_COMPONENTS}" | grep -qw "eck-elasticsearch"; then
  COMPONENT_WAIT_ARGS=""
  echo "  (async component â skipping --wait)"
fi
helm upgrade --install eck-elasticsearch oci://dp.apps.rancher.io/charts/eck-elasticsearch/eck-elasticsearch \
  --version * \
  -n nvidia-blueprint-rag --create-namespace \
  -f "${SCRIPT_DIR}/eck-elasticsearch/values.yaml" \
  ${COMPONENT_WAIT_ARGS} \
  || helm_failed "eck-elasticsearch"
echo "Installing zipkin (nvidia-blueprint-rag)..."
COMPONENT_WAIT_ARGS="${WAIT_ARGS}"
if echo "${ASYNC_COMPONENTS}" | grep -qw "zipkin"; then
  COMPONENT_WAIT_ARGS=""
  echo "  (async component â skipping --wait)"
fi
helm upgrade --install zipkin oci://dp.apps.rancher.io/charts/zipkin/zipkin \
  --version * \
  -n nvidia-blueprint-rag --create-namespace \
  -f "${SCRIPT_DIR}/zipkin/values.yaml" \
  ${COMPONENT_WAIT_ARGS} \
  || helm_failed "zipkin"
echo "Installing opentelemetry-collector (nvidia-blueprint-rag)..."
COMPONENT_WAIT_ARGS="${WAIT_ARGS}"
if echo "${ASYNC_COMPONENTS}" | grep -qw "opentelemetry-collector"; then
  COMPONENT_WAIT_ARGS=""
  echo "  (async component â skipping --wait)"
fi
helm upgrade --install opentelemetry-collector oci://dp.apps.rancher.io/charts/opentelemetry-collector/opentelemetry-collector \
  --version * \
  -n nvidia-blueprint-rag --create-namespace \
  -f "${SCRIPT_DIR}/opentelemetry-collector/values.yaml" \
  ${COMPONENT_WAIT_ARGS} \
  || helm_failed "opentelemetry-collector"
echo "Installing kube-prometheus-stack (nvidia-blueprint-rag)..."
COMPONENT_WAIT_ARGS="${WAIT_ARGS}"
if echo "${ASYNC_COMPONENTS}" | grep -qw "kube-prometheus-stack"; then
  COMPONENT_WAIT_ARGS=""
  echo "  (async component â skipping --wait)"
fi
helm upgrade --install kube-prometheus-stack oci://dp.apps.rancher.io/charts/kube-prometheus-stack/kube-prometheus-stack \
  ...
  ${COMPONENT_WAIT_ARGS} \
  || helm_failed "kube-prometheus-stack"
